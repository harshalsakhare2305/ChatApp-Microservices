...
function MessageInput({selectedUser,message,setMessage,handleMessageSend}:MessageInputProps) {
    ...
    <form onSubmit={handleSubmit} className='flex flex-col gap-2 border-t border-gray-700 pt-2'>
        {
            ImageFile && (<div className='relative w-fit'>
                ...
                <button type='button' className='absolute -top-2 -right-2 bg-black rounded-full p-1' onClick={()=>setImageFile(null)}><X
                className='w-4 h-4'/>
                </button>
            </div>)
        }
